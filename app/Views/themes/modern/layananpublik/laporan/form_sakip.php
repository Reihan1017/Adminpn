...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <form action="<?= site_url('layananpublik/simpanSakip') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="id" value="<?= $laporan['id'] ?? '' ?>">

            <div class="form-group">
                <label class="font-weight-bold">Judul Laporan</label>
                <input type="text" name="judul_laporan" class="form-control" placeholder="Contoh: Cetak Biru Mahkamah Agung 2010-2035" value="<?= old('judul_laporan', $laporan['judul_laporan'] ?? '') ?>" required>
            </div>
            <div class="form-group">
            ...
            <a href="<?= site_url('layananpublik/sakip') ?>" class="btn btn-secondary">Batal</a>
        </form>
    </div>
</div>