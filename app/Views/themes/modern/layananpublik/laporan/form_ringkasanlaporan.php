...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <form action="<?= site_url('layananpublik/simpanRingkasanLaporan') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="id" value="<?= $laporan['id'] ?? '' ?>">

            <div class="form-group">
                <label class="font-weight-bold">Judul Laporan</label>
                <input type="text" name="judul_laporan" class="form-control" placeholder="Contoh: LKjIP Tahun 2025" value="<?= old('judul_laporan', $laporan['judul_laporan'] ?? '') ?>" required>
            </div>
            <div class="form-row">
            ...
            <a href="<?= site_url('layananpublik/ringkasanLaporan') ?>" class="btn btn-secondary">Batal</a>
        </form>
    </div>
</div>