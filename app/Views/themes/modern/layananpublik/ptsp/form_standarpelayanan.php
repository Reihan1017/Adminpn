<?php if(session()->getFlashdata('errors')): ?>
    <div class="alert alert-danger shadow-sm">
        <strong>Gagal menyimpan! Periksa kembali isian Anda:</strong>
        <ul>
...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <form action="<?= site_url('layananpublik/simpanStandarPelayanan') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="id" value="<?= $dokumen['id'] ?? '' ?>">
            
            <div class="form-group">
                <label class="font-weight-bold">Judul Dokumen</label>
                <input type="text" name="judul_dokumen" class="form-control" value="<?= old('judul_dokumen', $dokumen['judul_dokumen'] ?? '') ?>" required>
            </div>
