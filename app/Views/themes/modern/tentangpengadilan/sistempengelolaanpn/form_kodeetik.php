...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <form action="<?= site_url('tentangpengadilan/simpanKodeEtik') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="id" value="<?= $dokumen['id'] ?? '' ?>">

            <div class="form-group">
                <label class="font-weight-bold">Judul Dokumen</label>
                <input type="text" name="judul_dokumen" class="form-control" placeholder="Contoh: 1. Kode Etik Hakim" value="<?= old('judul_dokumen', $dokumen['judul_dokumen'] ?? '') ?>" required>
            </div>
            ...
            <a href="<?= site_url('tentangpengadilan/kodeEtikHakim') ?>" class="btn btn-secondary">Batal</a>
        </form>
    </div>
</div>
